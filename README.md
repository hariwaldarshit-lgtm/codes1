...
A small Flask app that wraps `Qwen/Qwen2.5-1.5B-Instruct` behind a web chat UI. Deployable on [Hugging Face Spaces](https://huggingface.co/spaces) (free CPU tier, recommended — see below) or [Render](https://render.com).

## Deploy on Hugging Face Spaces (free, recommended)

Spaces' free CPU tier gives you 16GB RAM and 2 vCPUs — comfortably enough for this model, unlike most free hosts.

1. Go to https://huggingface.co/new-space
2. Pick a name, set **SDK** to **Docker**, set visibility, click **Create Space**
3. Push this whole project folder to the Space's git repo (it works exactly like GitHub):
   ```bash
   git remote add hf https://huggingface.co/spaces/<your-username>/<your-space-name>
   git push hf main
   ```
   (Or use the "Add file → Upload files" button in the Space's web UI if you'd rather not use git.)
4. The Space will build the `Dockerfile` automatically and start the app on port 7860 — no build command or port config needed, the `README.md` frontmatter at the top of this file already tells Spaces everything it needs.
5. Watch the **Logs** tab in your Space while it builds and starts — same idea as Render's logs.

That's it — no separate dashboard settings to configure, because Spaces reads its entire config from files in the repo.
